...
<body>
<div class="container">
	<form class="form-horizontal" id="myform" action="${pageContext.request.contextPath}/UserServlet?op=update&id=${user.id}" method="post">

		<div class="form-group">
			<label for="stuName" class="col-sm-2 col-sm-offset-3 control-label">用户名</label>
			<div class="col-sm-3">
				<input type="text" class="form-control" id="name" name="username" required="" value="${user.username}">
			</div>
		</div>
		...
		<div class="form-group">
			...
			<div class="col-sm-3">
				<input type="password" class="form-control" id="passowrd" name="password" required="" value="${user.password}">
			</div>
		</div>
